let input = `dark orange bags contain 3 dark chartreuse bags.
...
let bags : Map<string, Array<string>> = new Map();
...
while (prevFound.length != 0) {
    let found = []
    for (let check of prevFound)
        for (let [container, contained] of bags)
            if (contained.includes(check))
                found.push(container)
    allFound = allFound.concat(found)
    prevFound = found
}
console.log(new Set(allFound).size)